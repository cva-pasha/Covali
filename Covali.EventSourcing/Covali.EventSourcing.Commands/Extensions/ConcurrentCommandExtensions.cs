...
namespace Covali.EventSourcing.Commands.Extensions;
...
public static class ConcurrentCommandExtensions
{
    ...
    )
    {
        ...
        var tasks = handlers
            .Select(handler => (Task<TResult>)_bus.ExecuteAsync(
                    commandType,
                    command,
                    handler,
                    ct
    ...
    /// <summary>
    /// Resets the internal static command bus.
    /// This is intended for use in test scenarios to ensure test isolation.
    /// </summary>
    public static void ResetBus()
    {
        _bus.Dispose();
        _bus = new ConcurrentCommandBus();
    }
}